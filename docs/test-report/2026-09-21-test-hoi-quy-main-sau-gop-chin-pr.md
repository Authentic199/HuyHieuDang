# Báo cáo kiểm thử — chạy lại toàn bộ trên `main` sau khi gộp chín PR

Lệnh đã chạy: `cd BE && dotnet build` rồi `cd BE && dotnet test --no-build --logger trx`.

Tổng: 684 đạt, 0 hỏng, 7 bỏ qua (Skip) trên 691 ca.

Sáu dự án test: Core.UnitTests 140 đạt · Core.QcTests 118 đạt / 1 bỏ qua · Infrastructure.UnitTests 76 đạt · Infrastructure.IntegrationTests 1 đạt · Web.IntegrationTests 217 đạt · Web.QcIntegrationTests 132 đạt / 6 bỏ qua.

Bảy ca "bỏ qua" là các ca bị gắn `Skip` trong mã, không phải ca chạy xong. Sáu ca trong `A8DefectTests` mang lời hẹn "gỡ Skip khi PR vào main" — các PR đó đã vào main nhưng Skip chưa được gỡ; khi gỡ tay để thử thì hai ca đỏ (QC-T27-05 và QC-T27-07).
...
- BindToYear WhenTheFromDateIsAfterTheToDate ThrowsBadRequest — ĐẠT
