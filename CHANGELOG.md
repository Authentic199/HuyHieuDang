...
- **Bộ kiểm thử đầu-cuối chạy lại được ở mọi mốc thời gian.** Lớp fixture của ô chọn ngày dò tháng bằng chữ viết tắt tiếng Anh (`Jan`, `Feb`…), trong khi giao diện chạy locale `vi_VN` và lịch in "Th 01" — nên lịch không bao giờ chuyển tháng và mọi ca cần tháng khác tháng Một đều đỏ. Nay đọc tháng từ thuộc tính `title` của ô ngày, không phụ thuộc ngôn ngữ. Ba luồng `e1`, `e4`, `e7` xanh trở lại.
- **Gỡ nốt `Skip` ở tầng E2E.** Ca `E0-01` (Backend đóng băng "hôm nay") để `Skip` với lý do "Backend chưa đọc `HUYHIEUDANG_TEST_TODAY`" — lý do đã hết hiệu lực từ PR #38. Hai ca `E-903` / `E-904` vốn là chỗ trống không khẳng định gì, nay có thân thật và đã chạy xanh ở đúng mốc của chúng (15/10/2026 và 01/12/2026).
